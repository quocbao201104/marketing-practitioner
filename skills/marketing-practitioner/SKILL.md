...
For a complex marketing report or synthesis whose structure remains open, read [report planning and presentation](references/report-planning-and-presentation.md). For an HTML deliverable, use its HTML section only when implementation or delivery choices need guidance. A supplied sufficient outline stays on the direct path; this reference is not a new approval gate.
