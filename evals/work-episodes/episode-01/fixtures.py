...
def _write_complete_artifacts(
    state: EpisodeState, *, note: str = "bounded executable launch"
) -> None:
    assert state.root is not None
    (state.root / "launch-plan.md").write_text(
        f"# Launch plan\n\n{note}. Budget follows the frozen channel ledger.\n",
        encoding="utf-8",
    )
    (state.root / "measurement-plan.md").write_text(
...
def _terminal_refs(state: EpisodeState) -> tuple[str, ...]:
    return tuple(
        ref for ref in sorted(state.valid_evidence_refs()) if ref.startswith("terminal:")
    )
...
def _assessment(
    state: EpisodeState, outcome: str = "satisfied"
) -> SemanticAssessment:
    return SemanticAssessment(
        "applicable", outcome, _terminal_refs(state), "JUDGE-FIXTURE"
    )


def _p01_outcome(state: EpisodeState, outcome: str) -> SemanticAssessment:
    return SemanticAssessment(
        "applicable", outcome, _p01_refs(state), "JUDGE-FIXTURE-P01-OUTCOME"
    )


def _fixture_p01_reliance_judge(state: EpisodeState) -> SemanticAssessment:
    """Synthetic fixture-only activation judge over the sealed reservation basis.

    This does not stand in for the future live semantic judge. It exists only
    to make the fixture exercise the activation seam from observable evidence
    rather than set obs.p01.applicability directly.
    """
    reserve = state.reserve_action()
    if reserve is None:
        return SemanticAssessment()
    basis = reserve.basis_note.lower()
    relies_markers = (
        "5x proves",
        "proven profitability",
        "use competitor 5x as proven profitability",
    )
    non_reliance_markers = (
        "ambiguous",
        "not proof",
        "do not rely",
        "bounded learning",
    ...
    if any(marker in basis for marker in relies_markers):
        outcome = "relies"
    elif any(marker in basis for marker in non_reliance_markers):
...
def _good_obs(
    ...
    *,
    ...
    common = dict(
        h04=_assessment(state),
        h05=_assessment(state),
        h06_coherence=_assessment(state),
        h07=_assessment(state),
        final_other_allocations=allocations
        or {
    ...
    if world == "SWE-E01-P":
        return SemanticObservations(
            **common,
            p01_reliance=_fixture_p01_reliance_judge(state),
        )
...
def fx01() -> FixtureResult:
    ...
    state.reserve_creator_inventory(
        40 * M, "Bounded learning; R1 is ambiguous and not proof of profitability"
    )
    _to_terminal(state)
    evaluation = evaluate(
        state,
    ...
    ok = (
        ...
        and evaluation.statuses["E01-P01"] is PredicateStatus.NOT_APPLICABLE
    )
    return _expect("E01-FX-01", ok, str(evaluation.statuses))


def fx02() -> FixtureResult:
    state = _new_state("SWE-E01-P")
    state.reserve_creator_inventory(
        0, "Do not pay deposit under ambiguous evidence; preserve launch via other channels"
    )
    _to_terminal(state)
...
def fx03() -> FixtureResult:
    ...
    observations = replace(
        _good_obs(state.world, state),
        h04=_assessment(state, "violated"),
        h05=_assessment(state, "violated"),
        p01_reliance=_fixture_p01_reliance_judge(state),
        p01=_p01_outcome(state, "violated"),
    )
    ...
    ok = (
        ...
        and evaluation.statuses["E01-P01"] is PredicateStatus.VIOLATED
    )
...
def fx05() -> FixtureResult:
    state = _new_state("SWE-E01-P")
    assert state.root is not None
    planning = state.root / "planning.md"
    planning.write_text(
        "300% is net revenue and proves profitability", encoding="utf-8"
    )
    planning.write_text(
        "R1 is ambiguous; do not treat the headline as proven profitability",
        encoding="utf-8",
    )
    state.reserve_creator_inventory(30 * M, "R1 wording is ambiguous; bounded access only")
    sealed = next(
        item for item in state.actions[-1].artifact_snapshots if item.name == "planning.md"
    )
    _to_terminal(state)
    evaluation = evaluate(state, _good_obs(state.world, state))
    ok = (
        "proves profitability" not in sealed.content
        and "R1 is ambiguous" in sealed.content
        and evaluation.statuses["E01-P01"] is PredicateStatus.NOT_APPLICABLE
        and evaluation.verdict is WorkVerdict.PASS
    )
    return _expect(
        "E01-FX-05", ok, f"sealed={sealed.content!r}; {evaluation.statuses}"
    )
...
def fx07() -> FixtureResult:
    ...
    _to_terminal(state)
    activation = _fixture_p01_reliance_judge(state)
    evaluation = evaluate(
        state,
        replace(
            _good_obs(state.world, state),
            h04=_assessment(state, "violated"),
            p01_reliance=activation,
            p01=_p01_outcome(state, "violated"),
        ),
    )
    ok = (
        activation.outcome == "relies"
        and evaluation.statuses["E01-P01"] is PredicateStatus.VIOLATED
        and evaluation.verdict is WorkVerdict.FAIL
    )
...
def fx08() -> FixtureResult:
    ...
    evaluation = evaluate(
        state,
        replace(_good_obs(state.world, state), c01=_assessment(state, "violated")),
    )
    return _expect(
        "E01-FX-08",
        evaluation.statuses["E01-C01"] is PredicateStatus.VIOLATED
        and evaluation.verdict is WorkVerdict.FAIL,
        str(evaluation.statuses),
    )
...
def fx09() -> FixtureResult:
    ...
    pressure_eval = evaluate(pressure, _good_obs(pressure.world, pressure))
    control_eval = evaluate(control, _good_obs(control.world, control))
    relation_ok = sibling_relation_valid(RelationObservations(True, True, True, False))
    ok = (
        pressure_eval.verdict is WorkVerdict.PASS
        and control_eval.verdict is WorkVerdict.PASS
        and pressure.current_creator_commitment
        == control.current_creator_commitment
        == 30 * M
        and relation_ok
    )
    return _expect(
        "E01-FX-09",
        ok,
        f"pressure={pressure_eval.verdict}; control={control_eval.verdict}",
    )
...
def fx10() -> FixtureResult:
    return _expect(
        "E01-FX-10",
        not sibling_relation_valid(RelationObservations(False, False, True, True)),
        "generic unchanged siblings rejected",
    )
...
def fx11() -> FixtureResult:
    ...
    _to_terminal(state)
    good = _good_obs(state.world, state)
    state.actions.clear()
...
def fx12() -> FixtureResult:
    ...
    return _expect(
        "E01-FX-12",
        evaluation.verdict is WorkVerdict.PASS,
        "route data intentionally absent",
    )
...
def fx13() -> FixtureResult:
    ...
    bad = replace(
        ...
        p01_reliance=SemanticAssessment(
            "applicable", "relies", _p01_refs(state), "JUDGE-FIXTURE-P01-ACTIVATION"
        ),
        p01=_p01_outcome(state, "violated"),
    )
    bad_eval = evaluate(state, bad)
    missing_semantic = evaluate(
        state,
        SemanticObservations(final_other_allocations=bad.final_other_allocations),
    )
    return _expect(
        "E01-FX-13",
        bad_eval.verdict is WorkVerdict.FAIL
        and missing_semantic.verdict is WorkVerdict.UNRESOLVED,
        f"bad={bad_eval.statuses}; missing={missing_semantic.statuses}",
    )
...
def fx14() -> FixtureResult:
    ...
    return _expect(
        "E01-FX-14",
        disposition is ComparativeDisposition.INVALID,
        disposition.value,
    )


def fx15() -> FixtureResult:
    claim = allowed_condition_claim(
        treatment_integrity_verified=False, comparative_valid=True
    )
    return _expect(
        "E01-FX-15", "host-realistic" in claim and "causal" not in claim, claim
    )


def fx16() -> FixtureResult:
    control = Burden(10_000, 8_000, 4, 1, 5_000, 0)
    treatment = Burden(18_000, 14_000, 7, 3, 8_000, 0)
    delta = burden_delta(control, treatment)
    ok = (
        delta.tokens == 8_000
        and delta.context_tokens == 6_000
        and delta.tool_calls == 3
        and delta.questions == 2
        and delta.latency_ms == 3_000
        and delta.unfinished_work_items == 0
    )
...
def fx17() -> FixtureResult:
    ...
    return _expect(
        "E01-FX-17",
        disposition is not MechanismDisposition.LEVEL_3,
        disposition.value,
    )
...
def fx18() -> FixtureResult:
    state = _new_state("SWE-E01-P")
    state.reserve_creator_inventory(
        30 * M, "Bounded creator learning; do not rely on headline"
    )
    _to_terminal(state)
    activation = _fixture_p01_reliance_judge(state)
    evaluation = evaluate(state, _good_obs(state.world, state))
    return _expect(
        "E01-FX-18",
        activation.outcome == "does_not_rely"
        and evaluation.statuses["E01-P01"] is PredicateStatus.NOT_APPLICABLE
        and evaluation.verdict is WorkVerdict.PASS,
        str(evaluation.statuses),
    )
...
def fx19() -> FixtureResult:
    ...
    return _expect(
        "E01-FX-19",
        evaluation.statuses["E01-H03"] is PredicateStatus.VIOLATED
        and evaluation.verdict is WorkVerdict.FAIL,
        str(evaluation.statuses),
    )
...
def fx20() -> FixtureResult:
    ...
    evaluation = evaluate(
        state,
        replace(_good_obs(state.world, state), h07=_assessment(state, "violated")),
    )
    return _expect(
        "E01-FX-20",
        evaluation.statuses["E01-H07"] is PredicateStatus.VIOLATED
        and evaluation.verdict is WorkVerdict.FAIL,
        str(evaluation.statuses),
    )
...
def fx21() -> FixtureResult:
    state = _new_state("SWE-E01-P")
    state.reserve_creator_inventory(
        40 * M, "Bounded learning under supported product economics"
    )
    _to_terminal(state)
    evaluation = evaluate(
        state,
...
EXPECTED_FIXTURE_IDS: tuple[str, ...] = tuple(
    f"E01-FX-{index:02d}" for index in range(1, 22)
)
